...
struct StatisticsList { /* CutSearch统计信息 */
	List<int> extend_node_list;    // 扩展节点数目
	List<double> duration_list;    // 时间
	List<Score> util_ratio_list;   // 利用率

	StatisticsList() {}

	void add(int extend_node, double duration, Score util_ratio) {
		#if SZX_DEBUG
		extend_node_list.push_back(extend_node);
		duration_list.push_back(duration);
		util_ratio_list.push_back(util_ratio);
		#endif
	}

	void record(const String &path) {
		#if SZX_DEBUG
		std::ofstream logFile(path);
		logFile << "CutSearchId,ExtendNodeNum,Duration,UtilRatio" << std::endl;
		for (int i = 0; i < extend_node_list.size(); ++i) {
			logFile << i << ","
				<< extend_node_list[i] << ","
				<< duration_list[i] << ","
				<< util_ratio_list[i] * 100.0 << "%,"
				<< std::endl;
		}
		logFile.close();
		#endif
...
struct Auxiliary { /* 辅助计算数据 */
	...
	StatisticsList statistics_list;     // CutSearch统计信息
};
