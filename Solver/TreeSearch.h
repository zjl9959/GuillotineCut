...
public:
    ...
    struct ChooseInfo { // auxiliary information for choose items.
        int num; // item placed in plate numbers.
        double rate; // best item belongs plate usage rate.
        double score; // score = (alpha * rate + (1 - num / generation)) / (alpha + 1).
        ChooseInfo(int number, double usage_rate, double info_score) :num(number), rate(usage_rate), score(info_score) {};
    };
...
protected:
    ...
    void saveItemDistribute(const String & filePath);
...
public:
    ...
    List<List<ChooseInfo>> choose_info; // choose_info[plateId][itemId].
